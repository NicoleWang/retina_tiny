...
def draw_caption(image, box, caption):
    ...
    cv2.putText(image, caption, (int(0.5*(b[0]+b[2])), int(0.5*(b[1]+b[3]))),cv2.FONT_HERSHEY_PLAIN,1,(0, 0, 255), 2)
...
for idx, imname in enumerate(namelist):
    ...
    with torch.no_grad():
        ...
        idxs = np.where(scores.cpu().numpy()>=0.3)
        ...
        for j in range(idxs[0].shape[0]):
            ...
            pred_class = int(classification[idxs[0][j]])
            if pred_class == 0:
                label_name = "%.2f"%score
